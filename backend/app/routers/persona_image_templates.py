...
from app.services.prompt_studio_renderer import (_get_font_for_asset, _lerp_channel, _parse_hex_color, _get_font_for_text, _download_bytes, _calculate_contrast_ratio, _assemble_from_llm_instructions, _fit_within, render_text_layer_pango, _assemble_template_image, _assemble_manual_template_preview, verify_pango_bengali, _pct, _resolve_font_path, analyze_zone_luminance, _font_candidates_for_script, _render_gradient_background, _load_background_asset_image, _detect_script, _relative_luminance, _layer_rotation_degrees, _first_option, _srgb_to_linear, register_fonts_with_fontconfig, _merge_llm_instructions_with_overrides, _get_font, _composite_layer_onto_base, _lerp_hex, _get_font_family_name, _image_to_png_bytes, PANGO_AVAILABLE)
    ...
    for layer in template_json.get("layers") or []:
        ...
        if layer_type == "text":
            ...
            if "description" not in layer:
                raise HTTPException(status_code=400, detail=f"Layer {layer_id} of type {layer_type} requires a 'description' string.")
        elif layer_type == "badge":
            if "badge_text" not in layer or "badge_icon" not in layer:
                raise HTTPException(status_code=400, detail=f"Layer {layer_id} of type badge requires 'badge_text' and 'badge_icon'.")
...
def build_image_instruction_prompt(
    ...
    for layer in layers:
        ...
        if layer_type == "text":
            ...
            icon_lines = [f"{layer_type.upper()} LAYER {layer_id}"]
            icon_lines.append(f"Provide a short, plain language 'description' (e.g., 'pizza slice' or 'happy face') for this {layer_type}.")
            user_parts.append("\n".join(icon_lines))

        elif layer_type == "badge":
            badge_lines = [f"BADGE LAYER {layer_id}"]
            badge_lines.append("Provide a 'badge_text' (short text) and a 'badge_icon' (short plain language description).")
            user_parts.append("\n".join(badge_lines))
    ...
    icon_layers = [l for l in layers if str(l.get("type") or "").lower() in ("icon", "emoji")]
    badge_layers = [l for l in layers if str(l.get("type") or "").lower() == "badge"]
    
    all_render_layers = text_layers + overlay_layers + logo_layers + frame_layers + shape_layers + divider_layers + icon_layers + badge_layers
    ...
    for idx, layer in enumerate(all_render_layers):
        ...
        if layer_type == "text":
            ...
            response_lines.append('      "description": "short plain language description",')
        elif layer_type == "badge":
            response_lines.append('      "badge_text": "short text",')
            response_lines.append('      "badge_icon": "short icon description",')
